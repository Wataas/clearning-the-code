        ...
        switch(pagina) {
            ...
            case 6: //Meu mapa
                ...
                while (jogando_mapa) {
                    ...
                    //Loops que apenas vai desenhar o mapa
                    for (int i = 0; i < 12; i++) {
                        cout << "      "; 
                        for (int j = 0; j < 7; j++) {
                    //Se for uma caixa E no gabarito for um alvo vai PINTA DE VERDE
                            if (mapa[i][j] == '@' && gabarito[i][j] == '~') {
                                cout << "\033[32m" << mapa[i][j] << "\033[0m ";
                            }
                    //Se for um alvo sozinho vai pintar de vermelho
                            else if (mapa[i][j] == '~') {
                                cout << "\033[31m" << mapa[i][j] << "\033[0m ";
                            }
                    //Qualquer outra coisa fica normal
                            else {
                                cout << mapa[i][j] << " ";
                            }
                        }
                    ...
                    dc();
                    cout << "[W, A, S, D] Mover  |  [Q] Desistir   [R] resetar" << endl;
                    //Configurando setinhas
                    char comando = getch();
            
                    // Próxima posição pretendida pelo jogador/ l=personagem
                    int proximo_x = Lx;
                    int proximo_y = Ly;
            
                    if (comando == 'w' || comando == 'W') {  proximo_x=proximo_x-2; }
                    else if (comando == 's' || comando == 'S') { proximo_x=proximo_x+2; }
                    else if (comando == 'a' || comando == 'A') {  proximo_y=proximo_y-2; }
                    else if (comando == 'd' || comando == 'D') { proximo_y=proximo_y+2; }
                    ...
                    int meio_x = (Lx + proximo_x) / 2;
                    int meio_y = (Ly + proximo_y) / 2;
                    //SE A PRÓXIMA POSIÇÃO FOR VAZIA OU UM ALVO
                    if ((mapa[proximo_x][proximo_y] == ' ' || mapa[proximo_x][proximo_y] == '~') and mapa[meio_x][meio_y] == ' ') {
                    //Restaura o que estava embaixo do player usando o segundo mapa
                        mapa[Lx][Ly] = gabarito[Lx][Ly];
                        Lx = proximo_x;
                        Ly = proximo_y;
                        mapa[Lx][Ly] = '7'; // Desenha o player no novo lugar
                    }
                    else if ((mapa[proximo_x][proximo_y] != ' ' || mapa[proximo_x][proximo_y] == '~') and mapa[meio_x][meio_y] == ' ') {
                    //Restaura o que estava embaixo do player usando o segundo mapa
                        mapa[Lx][Ly] = gabarito[Lx][Ly];
                        Lx = meio_x;
                        Ly =meio_y;
                    ...
                    //SE A PRÓXIMA POSIÇÃO FOR UMA CAIXA
                    else if (mapa[proximo_x][proximo_y] == '@') {
                    //Calcula a posição para onde a caixa vai ser empurrada
                    ...
                    //CONDIÇÃO DE VITÓRIA
                    //Verificar se ainda tem um ~ sem caixa encima
                    bool ganhou = true;
                   for (int i = 0; i < 12; i++) {       
                        for (int j = 0; j < 7; j++) {
                    //ganhou fica vitoria e so passa pra frente se o proximo loop nao tornar ele false
